...
export function PricingV1({
  ...
  return (
    <Section
      className={`py-12 sm:py-16 md:py-20 lg:py-28 bg-slate-50/50 border-t border-slate-100 ${className || ""
        }`}
    >
      <Container>
        {/* Header */}
          ...
          {plansData.map((plan: any) => {
            const features = Array.isArray(plan.features)
              ? plan.features.map((f: any) => {
                ...
                return extractPlainText(
                  f.description || f.title || [f]
                );
              })
              : [];

            const ctaObj =
              plan.cta || plan.button || plan.primaryButton;
            ...
            return (
              <Card
                ...
                className={`relative flex flex-col transition-all duration-300 rounded-2xl overflow-visible gap-0 py-0
                  ${plan.featured
                    ? "bg-cream! brightness-[0.97]! hover:bg-cream! hover:brightness-[0.92]! text-primary"
                    : "bg-cream hover:brightness-[0.97]!"
                  }`}
              >
                <CardHeader className="px-5 sm:px-6 md:px-8 pt-6 sm:pt-8 md:pt-10 pb-0 gap-0">
                  {/* Label */}
                  <div className="flex items-center gap-4 mb-3 sm:mb-4">
                    <p
                      className={`text-xs font-medium uppercase tracking-[0.15em] sm:tracking-[0.18em] ${plan.featured
                          ? "text-primary/90"
                          : "text-neutral-500"
                        }`}
                    >
                  ...
                  {/* Plan name */}
                  <CardTitle
                    className={`text-xl sm:text-2xl md:text-2xl font-medium tracking-tight mb-1.5 sm:mb-2 ${plan.featured
                        ? "text-primary"
                        : "text-foreground"
                      }`}
                  >
                  ...
                  {/* Subtitle */}
                  <p
                    className={`text-xs sm:text-sm md:text-base font-medium leading-relaxed ${plan.featured
                        ? "text-primary/90"
                        : "text-neutral-500"
                      }`}
                  >
                ...
                {/* Divider */}
                <div
                  className={`mx-5 sm:mx-6 md:mx-8 my-5 sm:my-6 md:my-7 h-px ${plan.featured
                      ? "bg-primary/15"
                      : "bg-slate-100"
                    }`}
                />
                ...
                <CardContent className="px-5 sm:px-6 md:px-8 pb-0 flex-1">
                  <ul className="flex flex-col gap-3 sm:gap-4">
                    {features.map((f: string) => (
                      ...
                      >
                        <span
                          className={`shrink-0 flex size-4.5 sm:size-5 items-center justify-center rounded-full mt-0.5 ${plan.featured
                        ...
                        <span
                          className={`text-xs sm:text-sm md:text-base font-medium leading-snug ${plan.featured
                              ? "text-neutral-700"
                              : "text-neutral-600"
                            }`}
                        >
                ...
                <CardFooter className="px-5 sm:px-6 md:px-8 pb-6 sm:pb-8 md:pb-9 pt-6 sm:pt-7 md:pt-8 border-0 bg-transparent w-full">
                  <Button
                    asChild
                    variant="default"
                    className={`w-full py-2.5 flex items-center justify-center rounded-md font-semibold text-xs sm:text-[13px] transition-all duration-200 active:scale-[0.98] ${plan.featured
                        ? "bg-primary! text-white! hover:bg-neutral-800! shadow-xs"
                        : "bg-white text-primary hover:bg-white/50 shadow-sm"
                      }`}
                  >
                    <Link
                      href={ctaLink}
