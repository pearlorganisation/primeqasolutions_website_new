      ...
      <Container>
        ...
        {stats && stats.length > 0 && stats.some(stat => stat.value || stat.label) && (
          <div className="grid grid-cols-2 md:grid-cols-4 mb-20">
            {stats.filter(stat => stat.value || stat.label).map((stat, idx) => (
              <div
                key={stat.label}
                className={`flex flex-col items-center text-center bg-[var(--color-page-canvas)] py-6 ${idx % 2 === 0 ? "border-r border-neutral-100" : ""
                  } ${idx < 2 ? "border-b md:border-b-0 border-neutral-100" : ""
        ...
        {items.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-12 gap-y-16">
            {items.map((item) => (
              ...
              >
                <div className="mb-6">
                  <div className="flex size-12 items-center justify-center rounded-xl bg-neutral-50 text-primary transition-all duration-300 group-hover:bg-primary group-hover:text-white group-hover:scale-110 shadow-xs">
                    {item.iconImage?.url ? (
                      <Image
                ...
                <div className="flex flex-col gap-1.5">
                  <H3 className="text-lg"><HighlightedText text={item.title} /></H3>
                  <P><HighlightedText text={item.description} /></P>
                </div>
